...
var url = Ti.App.Properties.getString("acs-service-baseurl-Cafes");

if(!url) throw new Error("Url not found by acs-service-baseurl-Cafes.");
...
exports.services_getCoffee = function(data, cb) {
   var path = [];
   path.push('/getCoffee');
   InvokeService(path.join(''), "GET", data, cb);
};

exports.services_getDetails = function(data, cb) {
   var path = [];
   path.push('/getDetails');
   InvokeService(path.join(''), "GET", data, cb);
};
